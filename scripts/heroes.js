...
function getCharacter(idCharacter){
	return makeApiCall("characters/"+idCharacter);
}
...
var HeroDetails = React.createClass({
	loadHeroById: function(){
		getCharacter(currentId).then(function(data){
			this.setState({data: data.data.results});
		}.bind(this));
	},
	goBack: function(){
		React.renderComponent(new HeroBox(), document.getElementById('content'));
	},
	getInitialState: function(){
		return{ data: []};
	},
	...
	render: function(){
		...
		return(
            <div className="hero">
				<div className="heroWrapperDetails">
					<h2 className="heroName">{currentData.name}</h2>
					<div className="imageWrapper">
						<img src={currentData.thumbnail.path + "." + currentData.thumbnail.extension} className="heroThumbnail" />
					</div>
				...
				<button type="button" className="addToTeam btn btn-primary" onClick={this.goBack}>Back</button>
			</div>
		)
	}
});
...
var HeroList = React.createClass({
	...
	render: function(){
		...
		var nodes = this.props.data.map(function(hero, index){
			return <Hero id={hero.id} key={index} name={hero.name} thumbnail={hero.thumbnail} description={hero.description} addToTeam={that.addToTeam}></Hero>;
		});
...
var Hero = React.createClass({
	...
	viewDetails: function(){
		...
		React.renderComponent(new HeroDetails(), document.getElementById('content'));
	},
	render: function(){
		return (
			<div className="hero col-md-3">
				<div className="heroWrapper">
					...
					<button type="button" className="viewDetails btn btn-primary" onClick={this.viewDetails}>View Details</button>
